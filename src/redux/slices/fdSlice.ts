...
const fdCalculatorSlice = createSlice({
  ...
  reducers: {
    fdCalculator(state, action: PayloadAction<FixedDepositData>) {
      const { principal_amount, interest_rate, tenure, compound_type } =
        action.payload;
      ...
      let principal_increment_rate =
        1 +
        ((interest_rate * compound_time_per_year) / 100) *
          compound_time_per_year;
      let compound_interest =
        principal_amount *
        Math.pow(principal_increment_rate, compound_time_per_year * tenure);
