        ...
        {LinkData.map((item: InvestmentToolsLinks) => {
          return (
            <Link
              key={item.id}
              href={item.href}
              className="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium uppercase rounded-lg lg:text-xl  sm:text-lg px-5 py-2.5 text-center me-2 mb-2 flex items-center justify-center h-36 sm:h-"
            >
              {item.title} <span className="pl-2">{item.icon}</span>
            </Link>
          );
        })}
      </div>
