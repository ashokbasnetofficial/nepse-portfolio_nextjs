...
const sell_share_result:shareSellSummary = useSelector((state:any)=>state.sell)
const buy_share_result:BuyShareResult= useSelector((state:any)=>state.buy);
...
const FormComponent = formComponents[investmenttools_id];
    return <>
        ...
        <div className="flex items-center justify-center flex-col gap-10">
            <p className=" md:text-3xl sm:text-xl font-bold mt-20">404! Page Not Found.</p>
            <button onClick={routerHandler} className="bg-navy-blue px-5 py-2 rounded text-white">Back</button>
        </div>)
        :
       ( <>
        <h4 className="bg-gradient-to-r from-blue-800 to-blue-600 my-5 text-2xl font-bold text-white rounded w-full h-auto uppercase text-left py-5 pl-5">Calculator</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {Object.keys(results).map((key, index) => (
                  <div key={index}>
                    ...
                    {typeof results[key] !== 'string' && results[key]!==undefined ? (
                        `Rs. ${formatNumber(results[key])}`
                    ) : (`${results[key]}`)}
                </p>

                    {index < Object.keys(results).length - 1 && <hr className="mb-2 border-2 border-white" />}
                  </div>
                ))}
              </div>
            </div>
